from utils import get_datetime_now_str, get_clf_name
import subprocess
import copy
import json
import os
...
class ClassifierManager():
    ...
    def prepare_log_folders(self, clf_class, dataset_path):
        ...
        # Create folders
        folder = os.path.join(self.log_folder,
                              run_folder_name,
                              clf_folder_name)
    ...
    def run_weka_classifier(self, clf_class, clf_args, dataset_path):
        ...
        run_args += [
            '-classifications',
            'weka.classifiers.evaluation.output.prediction.CSV -p 1-last -file {0} -suppress'.\
                format(os.path.join(log_folder, 'predict.csv'))
        ]
        ...
        run_args = ['java', '-Xmx1024m', '-cp', self.weka_jar_path, clf_class] + run_args
