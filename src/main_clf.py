...
from utils import load_config_clf, load_config_data,\
    yield_classifiers
...
parser.add_argument('-cc', '--config-clf',
                    type=_valid_config_path,
                    help='path to classifiers config file',
                    required=True)
parser.add_argument('-cd', '--config-data',
                    type=_valid_config_path,
...
for clf_name, clf_args in yield_classifiers(classifiers):
    for dataset_path in dataset_paths:
        clf_man.run_weka_classifier(clf_name,
                                    clf_args,
                                    dataset_path)
